...
- `market_context.macro_asof` — the **OBSERVATION DATE of each macro value**, same keys as `macro`. **Read it before you call any series "unchanged", "frozen", or "stale".** A repeated value is almost never a broken feed: the desk polls every 8h, but `FEDFUNDS` and `CPIAUCSL` are **MONTHLY** series (an identical reading across cycles hours apart is the only possible answer), and `DTWEXBGS`/`DGS10` are **DAILY with a publication lag**, so they legitimately repeat within a day and across weekends/holidays. The date is what separates the cases: "CPI is 3 weeks old **because CPI is monthly**" is normal; "the DAILY dollar index has not printed a new observation in 10 days" is a genuine staleness finding worth flagging in `key_points`. Judge staleness against the series' own CADENCE and its `macro_asof` date, never against how many cycles you have personally seen the same number. (cy325-327: a prior read called DTWEXBGS a frozen feed across five cycles spanning under two days — an unfalsifiable claim at the time, because the date was being discarded before it reached you.)
