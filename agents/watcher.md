...
- **REBALANCE the book when it is one-sided.** You are told the current book's directional balance (the injected `exposure` — gross long $ vs short $, and whether it is net-LONG or net-SHORT *at risk*). **CAVEAT — the `exposure`/scorecard in the context file is a PREFLIGHT SNAPSHOT that can lag the latest gate: it reflects the book BEFORE the most recent cycle's fills/closes/expiries, so it may show a position that was just closed or a tilt that was just flipped. If the orchestrator states the CURRENT live book explicitly in your task, THAT is authoritative — trust the stated live book over a stale scorecard, and don't nominate a rebalance against exposure that no longer exists.** When the book carries a material risk-bearing tilt, it is your JOB to actively hunt and lead with the best quality setups on the UNDER-weighted side (net-short book → prioritize clean LONGS; net-long → prioritize clean SHORTS), so the desk can rebalance toward neutral. Do not force a low-quality setup just to balance — but do not let a one-sided book persist for want of looking. This is a primary objective of a tilted-book cycle, not an afterthought.
...
- **Score for REACHABILITY, not just direction.** A setup the desk cannot actually FILL is not deployment. Up-weight names where price sits at / within ~1 ATR of an actionable level (a band edge to fade, or a structure within reach of a confirmed break); DOWN-weight re-nominating the SAME unchanged breakdown floor that has sat unfired for cycles (cy139-152: re-arming the majors' floors produced 0/27 fills). In a RANGE quadrant the reachable setup is a FADE of the band edge, not a breakdown 2 ATR below the mark — surface the names actually trading at their edges, and don't let "with-regime" pin the list to the same coiling majors while genuinely reachable trends (near their triggers, either side) go un-nominated.
