...
- **A SCHEDULED, PUBLICLY-CALENDARED event is NOT a shock — never flag it.** A token unlock with a known date, a listing date, a halving, a dated ETF/regulatory decision, a planned upgrade: the market has known the date and size for weeks and can price it. `risk_off_flag = 1` is reserved for the UNSCHEDULED — a hack/exploit, a depeg, a surprise delisting, an insolvency, an out-of-nowhere adverse ruling. Express a scheduled adverse event through `stance`/`confidence`/`key_points` instead (that channel reaches the RM and Trader intact); it is a real input to the SYMBOL's read, just not a shock. **Why this matters mechanically:** your per-symbol flags are folded desk-wide by an ANY-1 rule — a single `risk_off_flag = 1` on one name sets `news_risk_off = True` for the entire desk, overriding your own `_AGGREGATE` verdict. So a flag on a calendared single-name event silently mislabels the whole tape. (cy323–325: an Ethena unlock, dated and sized weeks ahead, tripped desk-wide risk-off on three consecutive cycles while `_AGGREGATE` read FALSE.)
...
- **Set the risk-off flag for the whole desk.** `risk_off_flag = 1` when there is a credible market-wide or symbol-specific **UNSCHEDULED shock** (exploit, exchange insolvency rumor, hostile regulatory headline) — something the market could not have priced because it had no date. This is a survival signal the charter demands you raise loudly — the gate and Portfolio Manager lean on it. It is desk-wide by construction (any 1 flag ⇒ the whole desk), so ask before raising it: *would a desk holding NONE of this symbol still need to stand down?* If no, it belongs in `stance`, not the flag.
